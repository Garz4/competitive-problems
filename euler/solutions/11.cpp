...
static constexpr size_t quantity = 4;
...
static inline void compute_direction(
  ...
  for (i = x, j = y;
      i >= 0 && i < size && j >= 0 && j < size && current_quantity--;
      i += x_add, j += y_add) {
    current_product *= matrix[i][j];
  }

  // Make sure we covered all 4 numbers before setting the new greatest product.
  if (current_quantity == -1) {
    greatest_product = std::max(current_product, greatest_product);
  }
}
...
static inline void compute_all_products(const size_t& x, const size_t& y) {
  ...
  // Up-right
  compute_direction(x, y, -1, 1);

  // Right
  compute_direction(x, y, 0, 1);
